...
export const meta = {
  ...
  version: '3.0.0',
  author: 'selov',
  description: 'Get random videos (premium only)',
  guide: ['/corn'],
  cooldown: 5,
  type: 'premium',
  category: 'video'
...
// Track sent videos globally to avoid repeats
if (!global.cornSentVideos) global.cornSentVideos = [];

// Video links list (Google Drive direct download URLs)
const videoLinks = [
...
export async function onStart({ response, bot, chatId, messageID }) {
  try {
    if (!videoLinks.length) {
      return response.reply('❌ No video links available.');
    }

    // Filter out already sent videos
    let availableVideos = videoLinks.filter(video => !global.cornSentVideos.includes(video));

    // Reset if all videos have been sent
    if (availableVideos.length === 0) {
      global.cornSentVideos = [];
      availableVideos = videoLinks;
    }

    // Pick a random video
    const randomVideo = availableVideos[Math.floor(Math.random() * availableVideos.length)];
    global.cornSentVideos.push(randomVideo);

    const waitingMsg = await response.reply('⏳ Downloading video...');

    const cacheDir = path.join(__dirname, '..', 'cache', 'corn');
    await fs.ensureDir(cacheDir);
    const videoPath = path.join(cacheDir, `corn_${Date.now()}.mp4`);
